...
const ClassesCls: React.FC<ProjectProps> = ({ params }) => {
  ...
  const fetchClass = useCallback(async () => {
    try {
      if (type) {
        ...
        const renamedData = {
          ...data,
          label: data.getAllClass ? data.getAllClass : []
        };
        // if (type === 'classification') {
        //   setTypedata(renamedData.strClass ?? []);
        // } else {
        //   setTypedata(renamedData.label);
        //   console.log('typedata2 ', renamedData);
        // }
      ...
      { type === 'classification' && typedata.map((ctype, index) => (
        <div key={ctype.class_index} className="mb-4">
          <div onClick={() => selectIndex(ctype.class_index)} className={`flex pl-6 pr-6 space-x-4 hover:bg-orange-300 w-full text-left bg-opacity-50 rounded-2xl transition-colors ${selectedClassId === type.class_index ? "bg-orange-200" : ""}`}>
            ...
            <button
              onClick={() => toggleDropzone(ctype.class_index)}
            ...
            >
              {dropzoneVisibility[ctype.class_index]} Upload Images
            </button>
          </div>
          ...
          { images.length > 0 && index == 0 && (
            ...
            {images.map((url, index) => (
              <li  key={index} className="flex-none w-1/12 p-1">
                <img
                  src={process.env.ORIGIN_URL+'/img/'+params.id+'/classification/'+ctype.class_index+'/'+url}
                  alt={`Image ${index}`}
                  width={50}
                  height={50}
                  sizes="50vw"
